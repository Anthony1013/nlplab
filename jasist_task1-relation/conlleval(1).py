...
import Logger #jeffrey
log = Logger.main() #jeffrey
...
def evaluate(iterable, options=None):
    ...
    for row_index , line in enumerate(iterable):
        line = line.rstrip('\r\n')
        ...
        if num_features is None:
            ...
            log.warning(FormatError(('unexpected number of features: %d (%d)' %
                              (len(features), num_features)))) #j
            log.warning('row_index:{}\ttag:{}'.format(row_index,line)) #j
            continue
        ...
        if len(features) < 3:
            log.warning(FormatError('unexpected number of features in line %s' % line)) #j
            log.warning('row_index:{}\ttag:{}'.format(row_index,line)) #j
...
def main(argv):
    ...
    report(counts)
    log.info('"conlleval.py" {} has run over.'.format(args.file))
